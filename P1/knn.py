...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    #         dists[i, j] = torch.sum(torch.abs(x_test[j]-x_train[i]))

    for train_idx in range(num_train):
        for test_idx in range(num_test):
            dists[train_idx, test_idx] = torch.square(
                x_train[train_idx]-x_test[test_idx]).sum()
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # fx_trains = x_train.reshape(num_train, -1)
    # fx_tests  = x_test.reshape(num_test, -1)
    
    # for i in range(num_train):
    #     dist = torch.abs(fx_tests - fx_trains[i]).sum(1)
    #     dists[i, :] = dist
        
        
    for i in range(num_train):
        dists[i] = torch.sum(((x_test - x_train[i])**2).reshape(num_train, -1), dim=1).T
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # idea: (x - y)**2 = x^2 + y^2 - 2xy

    fx_train = x_train.reshape(num_train, -1)
    fx_test = x_test.reshape(num_test, -1)
    
    dists = (fx_train**2).sum(dim=1).reshape(-1, 1) + \
    		(fx_test **2).sum(dim=1).reshape(1, -1) - \
    		2 * torch.mm(fx_train, fx_test.T) 
...
class KnnClassifier:
    ...
    def predict(self, x_test: torch.Tensor, k: int = 1):
        ...
        dists_k_rank, dists_index = torch.topk(dists, k=k, dim=0, largest=False)
        y_test = self.y_train[dists_index]
	
		# # use median
		# y_test_pred, _ = y_test.median(dim=0)
		# y_test_pred = torch.ravel(y_test)
        
		# use unique and argmax
        values, indices = y_test.unique(return_counts=True, dim=0)
        y_test_pred = values[indices.argmax()]

        # y_test_pred = self.y_train[]
...
def knn_cross_validate(
    ...
    folds = torch.tensor(range(num_folds), dtype=torch.int64)
    ...
    for k in k_choices:
        ...
        for fold in range(num_folds):
            not_in_folds = tuple(folds[folds != fold].tolist())
            x_val_fold = (x_train_folds[fold])
            y_val_fold = (y_train_folds[fold])
            
            x_train_fold = torch.vstack(x_train_folds[:fold] + x_train_folds[fold+1:])
            y_train_fold = torch.hstack(y_train_folds[:fold] + y_train_folds[fold+1:])
...
#             print('y_val_fold:', y_val_fold.shape)
            ...
            clf = KnnClassifier(x_train_fold, y_train_fold)
            acc = clf.check_accuracy(x_val_fold, y_val_fold, k=k)
            acc_list.append(acc)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    accs = torch.tensor(list(k_to_accuracies.values()))
    k = list(k_to_accuracies.keys())
    
    
    mean_acc = torch.mean(accs, dim=1)
    best_k = k[mean_acc.argmax()]
